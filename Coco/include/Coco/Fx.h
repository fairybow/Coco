...
inline constexpr int clamp(int value) { return qBound(0, value, 255); }
...
inline auto contrast = [](double factor) {
    ...
    return [factor](QRgb pixel) {
        ...
        constexpr auto apply_ = [](int value, double factor) {
            auto normalized =
                (value - 128) / 128.0; // Force floating point division
            normalized *= factor;
...
inline auto tint = [](QColor tintColor, double strength) {
    ...
    return [tintColor, strength](QRgb pixel) {
        ...
        auto r = clamp(
            static_cast<int>(
                qRed(pixel) * (1 - strength) + tintColor.red() * strength));
        auto g = clamp(
            static_cast<int>(
                qGreen(pixel) * (1 - strength) + tintColor.green() * strength));
        auto b = clamp(
            static_cast<int>(
                qBlue(pixel) * (1 - strength) + tintColor.blue() * strength));
...
inline bool isDark(const QColor& color) noexcept
{
    return color.lightness() < 128;
}
inline bool isLight(const QColor& color) noexcept
{
    return color.lightness() >= 128;
}
...
inline QPixmap apply(const QPixmap& pixmap, FxOps... ops)
{
    ...
    for (auto y = 0; y < height; ++y) {
        auto original_line =
            reinterpret_cast<const QRgb*>(original.constScanLine(y));
...
inline QLinearGradient
bandedGradient(qreal x1, qreal y1, qreal x2, qreal y2, T&&... colors)
{
...
template <QColorConstructible... T>
inline QLinearGradient
bandedGradient(const QPointF& start, const QPointF& finalStop, T&&... colors)
{
...
inline QColor blend(const QColor& base, const QColor& overlay, qreal strength)
{
    ...
    return QColor(
        FxOp::clamp(
            int(base.red() * (1 - strength) + overlay.red() * strength)),
        FxOp::clamp(
            int(base.green() * (1 - strength) + overlay.green() * strength)),
        FxOp::clamp(
